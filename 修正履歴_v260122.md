...
## 🐛 追加バグ修正: SQLファイルがビルドされない

### 問題（Raspberry Piデプロイ時）

サーバーが繰り返しクラッシュ：

```
Error: ENOENT: no such file or directory, open '/home/sk283/tatekaezamurai-bot/server/dist/migrations/001_initial_schema.sql'
```

### 原因

TypeScriptコンパイラは `.ts` ファイルのみをコンパイルし、`.sql` ファイルは `dist/` ディレクトリにコピーされない。
...
"copy-sql": "mkdir -p dist/migrations && cp src/migrations/*.sql dist/migrations/"
```

**影響**: ビルド時に `src/migrations/001_initial_schema.sql` が `dist/migrations/` にコピーされる
