...
export class CommandHandler {
  ...
  private async handleStart(replyToken: string, groupId: string, userId: string): Promise<void> {
    // 既存セッション確認
    const existingSession = await firestoreService.getSession(groupId);
...
💡 使い方を見る → 「ヘルプ」`;
  ...
  private async handlePayment(
    ...
    const session = await firestoreService.getSession(groupId);
    ...
    if (!session.members.find(m => m.userId === userId)) {
      ...
      await firestoreService.updateSession(groupId, { members: session.members });
    }
    ...
    await firestoreService.addPayment(groupId, payment);
  ...
  private async handleSettle(replyToken: string, groupId: string): Promise<void> {
    ...
    // セッション更新
    await firestoreService.updateSession(groupId, {
      settlements,
      status: 'settled',
    });
  ...
  private async handleEnd(replyToken: string, groupId: string): Promise<void> {
    ...
    await firestoreService.endSession(groupId);
